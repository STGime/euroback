...
console.log(stats) // {'{'} total_users: 150, active_today: 23 {'}'}</pre>
		...
		<section id="edge-functions" class="scroll-mt-20">
			...
			<div class="space-y-4">
				...
				<p class="text-sm text-gray-700">Export your handler as <code>module.exports</code>. The runner passes <code>req</code> (a <code>Request</code>) and <code>ctx</code> (context).</p>
				<p class="text-sm text-amber-700 bg-amber-50 border-l-4 border-amber-400 px-3 py-2 mt-2 rounded"><strong>Heads-up:</strong> user code runs as a plain script, not an ES module. Don't use <code>import</code> or <code>export default</code> — they'll fail on parse.</p>
