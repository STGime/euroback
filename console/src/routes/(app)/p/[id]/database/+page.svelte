<script lang="ts">
	...
	function openInsertModal() {
		...
		for (const col of selectedSchema.columns) {
			if (col.default_value) continue; // skip columns with defaults
...
<div class="flex gap-6 h-[calc(100vh-13rem)]">
	...
	<div class="fixed inset-0 z-50 flex items-center justify-center p-4">
		...
		<div class="relative z-10 w-full max-w-lg max-h-[80vh] overflow-y-auto rounded-xl bg-white shadow-2xl">
			<div class="flex items-center justify-between border-b border-gray-200 px-6 py-4">
				<h2 class="text-lg font-semibold text-gray-900">Insert Row into {selectedTable}</h2>
				<button
					type="button"
					...
					<div>
						<div class="block text-sm font-medium text-gray-700 mb-1">
							...
							{#if col.default_value}
								<span class="ml-1 text-xs text-gray-300 font-normal">default: {col.default_value}</span>
							{/if}
						</div>
