<script lang="ts">
	...
	let rows = $derived([
		...
		{ label: 'Enforce SSO for org members (password sign-in refused when set)', free: false, pro: false, team: true, legal: true },
		...
		<div class="grid grid-cols-1 gap-6 md:grid-cols-2 lg:grid-cols-4">
			...
			<div class="relative rounded-2xl border {hasTeamBeta ? 'border-2 border-emerald-500 shadow-lg' : 'border-2 border-emerald-300 shadow-sm'} bg-white p-8">
				...
				<ul class="mt-6 space-y-2 text-sm text-gray-600">
					...
					<li class="flex gap-2"><span class="text-gray-400">•</span><span><strong>Organizations</strong> — invite teammates by email, org-level admin / member roles, org-owned projects (Team + Pro + Free under one bill)</span></li>
					<li class="flex gap-2"><span class="text-gray-400">•</span><span><strong>Enforce SSO for members</strong> — org-level switch: password sign-in refused when set</span></li>
