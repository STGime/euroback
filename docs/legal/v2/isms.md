...
- **Platform users** (Eurobase console operators, including customers who manage projects) can enrol **passkeys (WebAuthn)** on **every tier, Free included** — there is no per-tier gate. Passkeys are registered and asserted with `userVerification=required`, so a single assertion is phishing-resistant MFA (possession of the authenticator + biometric / device PIN). Once a user has at least one passkey, **MFA is on for that account**: a correct password alone no longer issues a session and must be followed by a passkey assertion, or the user signs in with the passkey directly. Passkeys are scoped to the console origin only (relying-party id `console.eurobase.app`, not the shared `eurobase.app` suffix that tenant apps use). Enrolment, removal, successful and failed passkey sign-ins are written to the tamper-evident audit log. Adding or removing a passkey requires the current password unless the session is under ten minutes old and was not minted by SSO (an organization's IdP must not be able to enrol a passkey on a member's personal account); wrong passwords there count against the same per-account sign-in failure limit, and Personal Access Tokens are refused.
  - **Recovery:** a user who has lost every passkey recovers through the emailed password-reset link. That reset removes all passkeys on the account (MFA off) and sends a security notice email. This is an accepted MVP trade-off: control of the account's mailbox is sufficient to recover the account. Recovery codes are a planned follow-up.
  - **SSO sessions:** for organizations signing in via SSO, MFA is delegated to the customer's identity provider; Eurobase adds no second step. Organizations that require SSO (`sso_required`) refuse password *and* passkey sessions for their projects. Optional verification that the IdP actually performed MFA (OIDC `acr`/`amr`) is tracked separately (#629), as is an org-level "require MFA for all members" policy (#628).
  - TOTP and SMS one-time codes are intentionally not offered: passkeys supersede TOTP for this use case, and SMS is a downgrade-attack surface.
