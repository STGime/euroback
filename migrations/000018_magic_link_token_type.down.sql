-- 000018_magic_link_token_type.down.sql
-- Remove 'magic_link' from email_tokens CHECK constraints.

BEGIN;

-----------------------------------------------------------------
-- 1. Revert existing tenant schemas: remove magic_link from CHECK
-----------------------------------------------------------------
DO $$
DECLARE
    rec RECORD;
...
BEGIN
    ...
    LOOP
        ...
        IF con_name IS NOT NULL THEN
            ...
            EXECUTE format(
                'ALTER TABLE %I.email_tokens ADD CONSTRAINT %I CHECK (token_type IN (''verification'',''password_reset''))',
                rec.schema_name, con_name
            );
        END IF;
    END LOOP;
...
-----------------------------------------------------------------
-- 2. Revert public.platform_email_tokens CHECK
-----------------------------------------------------------------
DO $$
DECLARE
    con_name TEXT;
BEGIN
    ...
    IF con_name IS NOT NULL THEN
        ...
        EXECUTE format(
            'ALTER TABLE public.platform_email_tokens ADD CONSTRAINT %I CHECK (token_type IN (''password_reset''))',
            con_name
        );
    END IF;
END;
...
-----------------------------------------------------------------
-- 3. Revert public.email_templates CHECK
-----------------------------------------------------------------
DO $$
DECLARE
    con_name TEXT;
BEGIN
    ...
    IF con_name IS NOT NULL THEN
        ...
        EXECUTE format(
            'ALTER TABLE public.email_templates ADD CONSTRAINT %I CHECK (template_type IN (''verification'',''password_reset'',''welcome'',''password_changed''))',
            con_name
        );
    END IF;
END;
...
-----------------------------------------------------------------
-- 4. Revert provision_tenant() to previous version (from migration 016)
-----------------------------------------------------------------
