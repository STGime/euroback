-- 000018_magic_link_token_type.up.sql
-- Add 'magic_link' to email_tokens CHECK constraints across all tenant schemas,
-- platform_email_tokens, email_templates, and provision_tenant().

BEGIN;

-----------------------------------------------------------------
-- 1. Alter existing tenant schemas: update email_tokens CHECK
-----------------------------------------------------------------
DO $$
DECLARE
    rec RECORD;
...
BEGIN
    ...
    LOOP
        -- Find the CHECK constraint name on token_type
...
-----------------------------------------------------------------
-- 2. Alter public.platform_email_tokens CHECK
-----------------------------------------------------------------
DO $$
DECLARE
    con_name TEXT;
...
-----------------------------------------------------------------
-- 3. Alter public.email_templates CHECK
-----------------------------------------------------------------
DO $$
DECLARE
    con_name TEXT;
BEGIN
    ...
    JOIN pg_namespace n ON r.relnamespace = n.oid
    WHERE n.nspname = 'public'
      AND r.relname = 'email_templates'
      AND c.contype = 'c'
      AND pg_get_constraintdef(c.oid) LIKE '%template_type%';

    IF con_name IS NOT NULL THEN
        EXECUTE format(
            'ALTER TABLE public.email_templates DROP CONSTRAINT %I', con_name
        );
        EXECUTE format(
            'ALTER TABLE public.email_templates ADD CONSTRAINT %I CHECK (template_type IN (''verification'',''password_reset'',''welcome'',''password_changed'',''magic_link''))',
            con_name
        );
    END IF;
END;
...
-----------------------------------------------------------------
-- 4. Update provision_tenant() to include 'magic_link' in CHECK
-----------------------------------------------------------------
...
BEGIN
    ...
    EXECUTE format(
        'CREATE TABLE %I.email_tokens (
            ...
            user_id     UUID        NOT NULL REFERENCES %I.users(id) ON DELETE CASCADE,
            token_hash  TEXT        NOT NULL,
            token_type  TEXT        NOT NULL CHECK (token_type IN (''verification'',''password_reset'',''magic_link'')),
            expires_at  TIMESTAMPTZ NOT NULL,
            used_at     TIMESTAMPTZ,
            created_at  TIMESTAMPTZ DEFAULT now()
        )',
