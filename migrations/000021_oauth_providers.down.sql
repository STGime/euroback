-- 000021_oauth_providers.down.sql
-- Remove OAuth provider columns from tenant user tables.
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP INDEX IF EXISTS %I.idx_users_provider', rec.schema_name);
        EXECUTE format('ALTER TABLE %I.users DROP COLUMN IF EXISTS provider_user_id', rec.schema_name);
        EXECUTE format('ALTER TABLE %I.users DROP COLUMN IF EXISTS provider', rec.schema_name);
    END LOOP;
END;
$$;

-- Revert provision_tenant() to the version without provider columns.
