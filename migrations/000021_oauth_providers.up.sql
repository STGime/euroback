-- 000021_oauth_providers.up.sql
-- Add OAuth provider columns to tenant user tables for social login (Google, GitHub).

BEGIN;

-- Add OAuth columns to existing tenant user tables.
DO $$
DECLARE
    rec RECORD;
BEGIN
    ...
    LOOP
        EXECUTE format('ALTER TABLE %I.users ADD COLUMN IF NOT EXISTS provider TEXT DEFAULT ''email''', rec.schema_name);
        EXECUTE format('ALTER TABLE %I.users ADD COLUMN IF NOT EXISTS provider_user_id TEXT', rec.schema_name);
        EXECUTE format('CREATE UNIQUE INDEX IF NOT EXISTS idx_users_provider ON %I.users(provider, provider_user_id) WHERE provider_user_id IS NOT NULL', rec.schema_name);
    END LOOP;
END;
$$;

-- Update provision_tenant() to include provider and provider_user_id in the users table.
