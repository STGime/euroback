-- 000022_auth_helpers.down.sql
-- Drop auth helper functions from all tenant schemas.
...
BEGIN
    ...
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS %I.auth_uid()', rec.schema_name);
        EXECUTE format('DROP FUNCTION IF EXISTS %I.auth_role()', rec.schema_name);
        EXECUTE format('DROP FUNCTION IF EXISTS %I.auth_email()', rec.schema_name);
    END LOOP;
END;
$$;

-- Revert provision_tenant() to version without auth helpers (from migration 000021).
