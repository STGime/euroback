-- 000022_auth_helpers.up.sql
-- Add GoTrue-compatible auth helper functions (auth_uid, auth_role, auth_email)
-- to each tenant schema for Supabase-compatible RLS policies.

BEGIN;

-- Add auth helpers to existing tenant schemas.
DO $$
DECLARE
    rec RECORD;
BEGIN
    ...
    LOOP
        -- auth_uid() — returns the current end-user's UUID
        EXECUTE format(
        ...
        -- auth_role() — returns the current auth role
        EXECUTE format(
        ...
        -- auth_email() — returns the current end-user's email
        EXECUTE format(
...
-- Update provision_tenant to include auth helper functions for new projects.
