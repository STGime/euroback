-- 000056_fix_provision_tenant_regression.down.sql
--
-- WARNING: reverting this hotfix REINTRODUCES the P0 regression from
-- 000055 — new tenants would again get vault_secrets.ciphertext (which
-- internal/vault/service.go cannot read) and a pre-000050 storage_objects
-- shape. Only run this to get back to the exact pre-000056 state.
--
-- This restores provision_tenant() to the (broken) 000055 body verbatim.
