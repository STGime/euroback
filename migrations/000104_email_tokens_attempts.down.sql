-- 000104_email_tokens_attempts.down.sql
--
-- Reverses 000104. Drops the `attempts` column + composite index from
-- every tenant's email_tokens, and rolls the provision_tenant function
-- back to the pre-#233 (000063) shape.
--
-- A rollback loses in-flight attempt counters for any active OTP —
-- surviving OTPs revert to the pre-fix tenant-wide match behaviour.
-- Only relevant if we've decided to abandon #233 rather than fix a
-- regression in it.

BEGIN;

-----------------------------------------------------------------
-- 1. Drop the composite index + attempts column per tenant
-----------------------------------------------------------------
DO $$
DECLARE
    rec RECORD;
BEGIN
    ...
    LOOP
        EXECUTE format(
            'DROP INDEX IF EXISTS %I.idx_email_tokens_user_type_expires',
            rec.schema_name
        );
        EXECUTE format(
            'ALTER TABLE %I.email_tokens DROP COLUMN IF EXISTS attempts',
            rec.schema_name
        );
    END LOOP;
END;
...
-----------------------------------------------------------------
-- 2. Restore the 000063 provision_tenant body verbatim
-----------------------------------------------------------------
