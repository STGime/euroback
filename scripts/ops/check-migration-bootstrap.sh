            ...
            - name: CHECK_SQL
              value: |
                ...
                SELECT 'member ' || m.rolname || ' IN ' || r.rolname || ': inherit=' || am.inherit_option || ' set=' || am.set_option || ' admin=' || am.admin_option
                 ...
                 WHERE r.rolname IN ('eurobase_migrator','eurobase_gateway','eurobase_developer','eurobase_function_runner')
                ...
                SELECT 'index: ' || indexdef FROM pg_indexes WHERE indexname = 'ix_retention_holds_lookup';
